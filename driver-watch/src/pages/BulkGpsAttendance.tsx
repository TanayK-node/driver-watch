...
import { Upload, Loader2, MapPin, Clock, Users } from "lucide-react";
...
export default function BulkGpsAttendance() {
  ...
  const handleSubmit = async () => {
    if (!file) {
      toast({ title: "Missing file", description: "Please upload a CSV file", variant: "destructive" });
      return;
    }
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Upload the GPS .csv export file. Date will be auto-detected from the timestamp column.</CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <CardContent>
            {data.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
                ...
                <p className="font-medium">No drivers found inside campus for uploaded timestamps</p>
              </div>
            ) : (
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  {data.map((row) => (
                    <TableRow key={`${row.driver_name}-${row.date}-${row.first_in}-${row.last_out}`}>
